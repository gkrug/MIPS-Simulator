...
bool Instruction::exec() {
  ...
  case JAL:
    ...
    break;
  case BEQ:
	//cout << "rs is " << reg[rs] << " rt is " << reg[rt] << endl;
	if (reg[rs] == reg[rt]) {
	   pc = pc + (signExtend16(imm) << 2) - 4;
   }
  ...
  case BNE:
   ...
   // pc = (reg[rs] != reg[rt]) ? pc + (signExtend16(target) << 2) - 4 : pc;
    // note pc was pre-incremented, need to subtract 4 from branch offset
    break;
  case ADDI:
